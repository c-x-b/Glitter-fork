...
public:
	...
	void GenerateTextureRecFromFloats(std::string texAlias, int width, int height, float *data);
	void BindTexture2D(std::string texAlias, std::string sampler, Shader &shader);
	void BindTextureRec(std::string texAlias, std::string sampler, Shader &shader);
